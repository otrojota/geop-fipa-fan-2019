...
class ProveedorCapasFIPAFAN2019 extends ProveedorCapas {
    constructor(opciones) {
        ...
        let capaMacrozonasSanitarias = new CapaObjetosConDatos("fipafan2019", "MacrozonasSanitarias", "Macrozonas Sanitarias", "subpesca", {
            ...
            configAnalisis:{
                ...
                analizadores:{
                    "serie-tiempo":{
                        nivelVariable:0,
                        tiempo:{tipo:"fijo", from:-7, to:0, temporalidad:"1M", fromDate:1262314800000, toDate:1451617199999},
                        variable:{
                            variableMinZ:"fipafan2019.abundancia_relativa",
                            acumulador:"max", 
                            codigo:"Abundancia Relativa->estacion",
                            filtroFijo:{ruta:"macrozona", valor:"${codigo-objeto}"},
                            filtros:[],
                        ...
                        variable2:{
                            variableMinZ:"fipafan2019.toxina_vdm",
                            acumulador:"max", 
                            codigo:"Toxina VDM->estacion",
                            filtroFijo:{ruta:"macrozona", valor:"${codigo-objeto}"},
                            filtros:[],
                            nombre:"Toxina VDM",
                            temporalidad:"1M",
        ...
        let capaEstaciones = new CapaObjetosConDatos("fipafan2019", "EstacionesMonitoreo", "FIPA - Estaciones de Monitoreo", "fipafan2019", {
            ...
            geoJSON:true,
            iconoEnMapa:"img/estacion-fipa.png",
            configAnalisis:{
                analizador:"serie-tiempo",
                analizadores:{
                    "serie-tiempo":{
                        ...
                        variable:{
                            ...
                            acumulador:"max", 
                            codigo:"Abundancia Relativa->estacion",
                            filtroFijo:{ruta:"estacion", valor:"${codigo-objeto}"},
                            filtros:[],
                            icono:"${fipafan2019}/img/abundancia_relativa.svg",
                            nombre:"Abundancia Relativa",
                            temporalidad:"1M",
                            tipo:"queryMinZ"
                        },
        ...
        try {
            switch(cmd) {
                ...
                case "importaClorofila":
                    this.importaClorofila();
                    res.status(200).send("Corriendo en background ...").end();
                    break;
                case "importaSalinidad":
                    this.importaSalinidad();
                    res.status(200).send("Corriendo en background ...").end();
                    break;
                case "importaViento":
                    this.importaViento();
                ...
                case "importaTemperaturaAmbiental":
                    await this.importaTemperaturaAmbiental();
                    res.status(200).send("Corriendo en background ...").end();
                    break;
                case "importaToxinaVDM":
                    this.importaToxinaVDM();
                    res.status(200).send("Corriendo en background ...").end();
                    break;
                case "importaAbundanciaRelativa":
                    this.importaAbundanciaRelativa();
                    res.status(200).send("Corriendo en background ...").end();
                    break;
        ...
        try {
            ...
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/oxigeno_disuelto.svg",
                }
            });
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/clorofila.svg",
                }
            })
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/salinidad.svg",
                }
            })
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/velocidad-viento.svg",
                }
            })
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/temperatura.svg",
                }
            })
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/toxina_vdm.svg",
                }
            })
            await minz.addOrSaveVariable({
                ...
                options:{
                    ...
                    icon:"${fipafan2019}/img/abundancia_relativa.svg",
                }
            })
        } catch(error) {
            throw error;
    ...
    async importaOxigeno() {
        const fs = require("fs");
        minz.setBatchOptions(500, {});
        try {
            console.log("\n\nImportando Oxigeno");
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                for (let i=0; i<prof.length; i++) {
                    ...
                    if (v) {
                        v = parseFloat(v);
                        if (!isNaN(v)) {
                            await minz.batchAccum("fipafan2019.oxigeno", time.valueOf(), v, {
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                for (let i=0; i<prof.length; i++) {
                    ...
                    if (v) {
                        v = parseFloat(v);
                        if (!isNaN(v)) {
                            await minz.batchAccum("fipafan2019.clorofila", time.valueOf(), v, {
                                estacion:codigoEstacion, profundidad:profundidad, programa:programa
                            });
                        }
                    }
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                for (let i=0; i<prof.length; i++) {
                    ...
                    if (v) {
                        v = parseFloat(v);
                        if (!isNaN(v)) {
                            await minz.batchAccum("fipafan2019.salinidad", time.valueOf(), v, {
                                estacion:codigoEstacion, profundidad:profundidad, programa:programa
                            });
                        }
                    }
        ...
        try {
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                if (v !== undefined && v.trim().length) {
                    v = parseFloat(v);
                    if (!isNaN(v)) {
                        await minz.batchAccum("fipafan2019.velocidad_viento", time.valueOf(), v, {
    ...
    async importaTemperaturaAmbiental() {
        minz.setBatchOptions(500, {});
        const fs = require("fs");
        try {
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                if (v !== undefined && v.trim().length) {
                    v = parseFloat(v);
                    if (!isNaN(v)) {
                        await minz.batchAccum("fipafan2019.temperatura_ambiental", time.valueOf(), v, {
    ...
    async importaToxinaVDM() {
        ...
        try {
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                if (vdm == "+" || vdm == "-") {
                    await minz.batchAccum("fipafan2019.toxina_vdm", time.valueOf(), (vdm == "+"?1:0), {
    ...
    async importaAbundanciaRelativa() {
        minz.setBatchOptions(500, {});
        const fs = require("fs");
        try {
            console.log("\n\nImportando Toxina VDM");
            ...
            for (let k=0; k < lineas.length; k++) {
                ...
                for (let i=0; i<especies.length; i++) {
                    ...
                    if (v !== undefined && v.trim().length) {
                        v = parseFloat(v);
                        if (!isNaN(v)) {
                            await minz.batchAccum("fipafan2019.abundancia_relativa", time.valueOf(), v, {
